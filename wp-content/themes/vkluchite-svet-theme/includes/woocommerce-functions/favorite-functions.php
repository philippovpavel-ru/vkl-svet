...
function vlksvet_wc_favorite()
{
  ...
  $favorite_list_array = vklsvet_wc_get_favorite_list() ?? [];
...
add_filter('woocommerce_account_menu_items', 'vlksvet_wc_favorite_list_link', 25);
function vlksvet_wc_favorite_list_link($menu_links)
{
...
add_action('init', 'vlksvet_wc_add_endpoint', 25);
function vlksvet_wc_add_endpoint()
{
  add_rewrite_endpoint('favorite-list', EP_PAGES);
}

// Добавляю поле настройки пользователю для хранения значений 
add_action('show_user_profile', 'vklsvet_wc_show_profile_fields');
add_action('edit_user_profile', 'vklsvet_wc_show_profile_fields');
function vklsvet_wc_show_profile_fields($user)
{
  $userID = $user->ID;
...
add_action('woocommerce_account_favorite-list_endpoint', 'vlksvet_wc_content', 25);
function vlksvet_wc_content()
{
...
add_action('wp_ajax_favorite_click', 'vlksvet_wc_favorite_click');
function vlksvet_wc_favorite_click()
{
  ...
  if (!in_array($product_id, $favorite_list_array)) {
    ...
    vklsvet_wc_update_favorite_list(implode(',', $favorite_list_array));
  } else {
...
function vklsvet_wc_get_favorite_list(): array
{
...
function vklsvet_wc_update_favorite_list(string $favorite_list)
{
  $userID = get_current_user_id();
